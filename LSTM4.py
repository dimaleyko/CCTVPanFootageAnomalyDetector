...
class RNN(nn.Module):
    ...
    def forward(self, x, xn, xnn,xnnn): #, xnn,xnnn, xnnnn
...
def evaluate(epoch, dataset, frames, trainloss, totalframes):
    ...
    with torch.no_grad():
        ...
        for file in range(10):
            ...
            imageout = loadsave.load_image(DIRV+'/output%06d.jpg' %(file+5)).to(device)


            outputs = model(imageinN1,imageinN2, imageinN3, imageinN4)#,imageinN3, imageinN4, imageinN5
...
for epoch in range(num_epochs):
    ...
    for folder in range(datasamples):
        ...
        for file in range(total_step):
            ...
            imageout = loadsave.load_image(DIRRT+'/output%06d.jpg' %(file+5)).to(device)
...
np.save("weights4", weights)
wandb.save("weights4.npy")
